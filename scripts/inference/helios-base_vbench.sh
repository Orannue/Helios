...
OUTPUT_DIR="${OUTPUT_DIR:-output_helios/vbench_all_dimension}"

# Local or Hugging Face model paths. Override these from CLI or environment.
BASE_MODEL_PATH="${BASE_MODEL_PATH:-BestWishYsh/Helios-Base}"
TRANSFORMER_PATH="${TRANSFORMER_PATH:-BestWishYsh/Helios-Base}"
LORA_PATH="${LORA_PATH:-}"
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/inference/helios-base_vbench.sh [options]
...
Examples:
  bash scripts/inference/helios-base_vbench.sh \
    --base-model-path /models/Helios-Base \
    --transformer-path /models/Helios-Base \
    --num-frames 99 \
    --output-dir output_helios/vbench_all_dimension

  bash scripts/inference/helios-base_vbench.sh \
    --extended-prompt-file vbench_prompts/all_dimension_longer.txt \
    --output-dir output_helios/vbench_all_dimension_longer
USAGE
}
